...
export class PagesController {
  ...
  async getCartPage(@Query('auth') auth?: string) {
    const recommendedProducts =
      await this.productsService.getRecommendedProducts();
  ...
  getAboutPage(@Query('auth') auth?: string) {
    return {
      ...
      metaDescription:
        'О магазине MusicStore - наша история, команда и философия',
      isAuthenticated: auth === 'true',
      cartCount: 0,
      useSwiper: false,
      useInputMask: false,
  ...
  getCreditPage(@Query('auth') auth?: string) {
    return {
      ...
      metaDescription:
        'Оформите кредит или рассрочку на покупку гитары в MusicStore',
      isAuthenticated: auth === 'true',
      cartCount: 0,
      useSwiper: false,
      useInputMask: true,
